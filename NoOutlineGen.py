...
# Date: 16/09/2025
# Description: Code for prompting LLaMA to generate seven paragraph stories without an event outline or phoneme incorporation.
...
# The theme file is the output file from theme extraction, the source file is the output file from Match.py 
# and the output file can be what you want.
THEME_FILE = ""
SOURCE_FILE = ""
OUTPUT_FILE = ""
...
# Method for reading in all the earlier extracted themes from which will select a random one to go with each initial paragraph. 
...
def generate_continuation_paragraph(
    ...
    # and transition to incorporate as well as the previous and current source paragraphs and the last paragraph generated
    # by the model.
    prompt_body = f"""
...
def build_story(
    ...
    # theme of the story and transition into the paragraph above each paragraph.
...
if __name__ == "__main__":
    ...
    for block in blocks:
        ...
        full_story = build_story(theme, initial_paragraph, paragraphs, transitions, target_paragraph_count=7)

        # Saves each generateed story.
        with open(output_file, "a", encoding="utf-8") as f_out:
            f_out.write(f"--- Generated Story {story_id} ---\n\n")
