...
# to use in the auto-regressive story generation. Version for single story, phoneme incorporation generation. 
...
# Method for reading in the paragraph generated in initial paragraph generation and extracting only the actual 
...
# Takes in the initial paragraph, the path to the source story dataset and the desired match score.
# Here the threshold is lower than for the larger scale generation, no phoneme generation side as was run on a laptop
# and there was not time nor processing power to have 0.8 as a threshold with a greater possibility of ending
...
if __name__ == "__main__":
    ...
    threshold = 0.6
    # Performs the scoring on the one generated initial paragraph. Keep this as one because need to perform full generation process
    # once at a time to simulate ART usage.
    NUM_MATCHES = 1 
