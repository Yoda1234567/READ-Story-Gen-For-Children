...
# Version for larger scale, no phoneme generation. 
...
# the event mapping file is the output file from event extraction and conversion and the output file can be what you want.
OUTLINE_FILE_PATH = ""
THEME_FILE_PATH = ""
EVENT_MAPPING_FILE = ""
OUTPUT_FILE = ""
...
# Method for reading in all the earlier extracted themes from which will select a random one to go with each outline for 
...
# Method for reading all the event-description mapping pairs for mapping the first event in a outline to its incorporation
...
# Method for reading all the selected outlines from which a single outline will be selected for each initial paragraph
...
def load_all_outlines(outline_file: str) -> list[list[str]]:
    ...
    try:
        ...
        with open(outline_file, "r", encoding="utf-8") as f:
            # Goes through all the outlines, separates each into a list of the events contained in it, and keeps them all.
            for line in f:
                line = line.strip()
                if not line or line.startswith('#'):
                    continue
...
def generate_paragraph_core(
    ...
    # and event that the model must incorporate into the paragraph.
    ...
    prompt = instruction_prompt_template.format(event=final_event_instruction, theme=theme_prompt)
...
# Method for taking in a single selected outline and generating the initial paragraph containing the first outline event.
...
if __name__ == "__main__":
    ...
    # Specifies the number of outlines to generate initial paragraphs for. Set this to however many stories 
    # you would like to eventually generate.
    num_stories = 50
